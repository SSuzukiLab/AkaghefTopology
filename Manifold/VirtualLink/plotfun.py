def plotfun():
    ...
    pd_code=self.pd_code()
    print(pd_code)
    regions=self.pd_code()
    print(regions)
    orientation=self.orientation()
    print(orientation)
    edges = list(set(flatten(pd_code)))
    
    # regions=[[6, -5], [5, 1, 3], [4, -3], [2, -1], [-2, -6, -4]]
    # edges=[1,2,3,4,5,6]
    regions = sorted(self.regions(), key=len)
    edges.sort()
        ...
        if len(im) == 4 and max(x[1] for x in im) == 1:
            ...
            while c < len(im)-1:
                ...
                if im[c+1][1] == 1 and c < len(im) - 2:
                    ...
                    coords.append(bezier_coords)
                    info.append([4,c,ce,c20])
                    p = q
                    c += 1
        # line([p, im[-1][0][1]], **kwargs)の代わりに座標を抽出
        coords.append([p, im[-1][0][1]])
        info.append([5,c,ce,c20])
        
        # 各セグメントの基本座標も追加
        # for a in im:
        #     coords.append(a[0])
    print(list(crossings.keys()))
    print(list(crossings.values()))
    Cr=[crossings[tuple(x)] for x in pd_code]
    return [coords,info,Cr]
...
# print(coords)
# print(coords)
[flatten(coords),coords,crossings,info]